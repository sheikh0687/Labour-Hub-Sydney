...
struct GetCompanyEmpProjects : Codable {
    let result : [ResCompanyEmpProject]?
    let status : String?
    let message : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([ResCompanyEmpProject].self, forKey: .result)
        status = try values.decodeIfPresent(String.self, forKey: .status)
        message = try values.decodeIfPresent(String.self, forKey: .message)
    }
}

struct ResCompanyEmpProject : Codable {
    let id : String?
    let store_id : String?
