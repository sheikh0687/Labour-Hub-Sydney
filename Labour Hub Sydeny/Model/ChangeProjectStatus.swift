...
struct GetProjectStatus : Codable {
    let result : ResProjectStatus?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(ResProjectStatus.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct ResProjectStatus : Codable {
    let id : String?
    let user_id : String?
    let title : String?
