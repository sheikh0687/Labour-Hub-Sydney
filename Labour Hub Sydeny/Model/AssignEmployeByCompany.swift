...
struct AssignEmployees : Codable {
    let result : ResAssignEmployee?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(ResAssignEmployee.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct ResAssignEmployee : Codable {
    let id : String?
